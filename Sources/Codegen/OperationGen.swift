...
/// Converts a regular object ``Decl`` generated by ``gen(object:named:typename:fragmentInfo:)`` into a ``SwiftUIGraphQL/QueryOperation`` or ``SwiftUIGraphQL/MutationOperation``.
func attach(operation: OperationDefinition, to decl: Decl, schema: GraphQLSchema, fragmentInfo: FragmentInfo) -> Decl {
    ...
    let operationProtocolName: String
    switch operation.operation {
    case .query:
        operationProtocolName = "QueryOperation"
    case .mutation:
        operationProtocolName = "MutationOperation"
    case .subscription:
        fatalError("Subscriptions aren't yet supported")
    }
    
    return .struct(
        name: name,
        decls: decls + extraDecls,
        conforms: [operationProtocolName] + conforms
    )
}
