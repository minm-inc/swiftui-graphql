...
func attach(operation: OperationDefinition, to decl: Decl, schema: GraphQLSchema, fragmentDefinitions: [FragmentDefinition]) -> Decl {
    ...
    let fragmentMap: OrderedDictionary<String, FragmentDefinition> = fragmentDefinitions.reduce(into: [:]) {
        $0[$1.name.value] = $1
    }
    ...
    extraDecls.append(.staticLetString(name: "query", literal: queryString))
